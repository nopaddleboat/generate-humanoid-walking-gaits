...
class FootSteps(object):
    ...
    def get_act_left_position(self, t):
        left=np.array(self.get_left_position(t))
        left_next=np.array(self.get_left_next_position(t))
        t_start=self.get_phase_start(t)
        t_total=self.get_phase_duration(t)
        k=(t-t_start)/t_total

        return  left+(left_next-left)*k
        
    def get_act_right_position(self, t):
        right=np.array(self.get_right_position(t))
        right_next=np.array(self.get_right_next_position(t))
        t_start=self.get_phase_start(t)
        t_total=self.get_phase_duration(t)
        k=(t-t_start)/t_total
        return  right+(right_next-right)*k
