...
class FactorGraph(object):
    ...
    Method solve() is used to compute the solution to the problem. solve_cvx() is faster than solve().
    '''
    ...
    def add_factor(self, factors, reference):
        ...
        for ref in reference:
            if self.b.size == 0:
                self.b = np.vstack([ref])
            else:
                self.b = np.vstack([self.b, ref])
    ...
    def add_factor_constraint(self, factors, reference):
        ...
        if self.d.size == 0:
            ...
            self.d = np.concatenate((self.d, reference))
    ...
    def solve(self, eps=1e-8):
        ...
        xopt = np.transpose(np.array([np.matmul(Cp , self.d)]))
        P = eye(self.nx * self.N) - np.matmul(Cp , self.C)
        xopt += np.matmul(npl.pinv(np.matmul(self.A , P), eps) , self.b - np.matmul(self.A , xopt))  
        return xopt
    
    def solve_cvx(self,eps=0.0005): #solve with cvxopt, much faster than the above solve     
        AT=np.transpose(np.copy(self.A))
        P=matrix(np.matmul(AT,self.A)+np.identity(self.A.shape[1])*eps)
        q= matrix(-np.matmul(self.b.transpose(),self.A).flatten())
        sol=solvers.qp(P=P,q=q,A=matrix(self.C),b=matrix(self.d))
        return sol['x']
